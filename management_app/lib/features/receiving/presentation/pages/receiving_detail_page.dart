...
class _ReceivingDetailPageState extends State<ReceivingDetailPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Load returns for this receiving
      _loadReturns();
    });
  }
  ...
  void _loadReturns() {
    context.read<PurchaseReturnBloc>().add(
      LoadPurchaseReturnsByReceivingId(widget.receivingId),
    );
  }
  ...
  Future<void> _printReceiving(Receiving receiving) async {
    // Get returns data
    final returnState = context.read<PurchaseReturnBloc>().state;
    final returns =
        (returnState is PurchaseReturnLoaded)
            ? returnState.purchaseReturns
            : <PurchaseReturn>[];

    final pdf = await _generatePdf(receiving, returns);
  ...
  Future<pw.Document> _generatePdf(
    ...
    final completedReturns =
        returns.where((r) => r.status.toUpperCase() == 'COMPLETED').toList();

    final totalReturn = completedReturns.fold<double>(
      0.0,
      (sum, r) => sum + r.total,
    );

    final finalTotal = receiving.total - totalReturn;
                ...
                child: pw.Column(
                  children: [
                    ...
                    if (completedReturns.isNotEmpty) ...[
                      ...
                      _buildSummaryRow('Total Return', -totalReturn),
                      pw.Divider(thickness: 2),
                      _buildSummaryRow('TOTAL AKHIR', finalTotal, isBold: true),
                    ],
                  ],
                ),
              ),
                              ...
                              return pw.Padding(
                                ...
                                child: pw.Row(
                                  ...
                                  children: [
                                    pw.Expanded(
                                      child: pw.Text(
                                        '• ${item.productName} (${item.returnQuantity} ${item.returnQuantity > 1 ? "items" : "item"})',
                                        style: const pw.TextStyle(fontSize: 9),
                                      ),
                                    ),
                                    pw.Text(
                  ...
                  children: [
                    ...
                    BlocBuilder<PurchaseReturnBloc, PurchaseReturnState>(
                      builder: (context, returnState) {
                        ...
                        final finalTotal = receiving.total - totalReturn;
                        final hasReturns = completedReturns.isNotEmpty;
                    ...
                    if (receiving.status.toUpperCase() == 'COMPLETED')
                      _buildReturnsSection(receiving),
  ...
  Widget _buildReturnsSection(Receiving receiving) {
                  ...
                  return InkWell(
                    onTap: () {
                      ...
                      Navigator.push(
                        ...
                        // Refresh returns after coming back
                        _loadReturns();
                      });
                    },
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              Text(
                                '${returnItem.items.length} item diretur',
                                style: TextStyle(
                                  fontSize: 12,
                                  color: Colors.grey.shade600,
                                ),
