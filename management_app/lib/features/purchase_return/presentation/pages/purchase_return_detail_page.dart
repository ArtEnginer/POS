...
class _PurchaseReturnDetailPageState extends State<PurchaseReturnDetailPage> {
  ...
  Future<void> _showStatusUpdateDialog(PurchaseReturn returnData) async {
    final currentStatus = returnData.status.toUpperCase();
    String? selectedStatus;

    // Define available status transitions
    List<String> availableStatuses = [];
    if (currentStatus == 'DRAFT') {
      availableStatuses = ['APPROVED', 'CANCELLED'];
          ...
          (context) => StatefulBuilder(
            builder:
                (context, setState) => AlertDialog(
                  title: const Text('Ubah Status Return'),
                  content: Column(
    ...
    if (result != null && mounted) {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder:
            (context) => AlertDialog(
              ...
              content: Text(
                'Apakah Anda yakin ingin mengubah status dari $currentStatus ke $result?',
              ),
              actions: [
                TextButton(
      ...
      if (confirmed == true && mounted) {
        context.read<PurchaseReturnBloc>().add(
          events.UpdatePurchaseReturnStatusEvent(returnData.id, result),
        );
      }
    }
  }

  Widget _buildStatusUpdateSection(PurchaseReturn returnData) {
    final currentStatus = returnData.status.toUpperCase();

    String nextStatusText = '';
    if (currentStatus == 'DRAFT') {
      nextStatusText = 'Approve atau Cancel return ini';
    } else if (currentStatus == 'APPROVED') {
      nextStatusText = 'Complete atau Cancel return ini';
    }
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: () => _showStatusUpdateDialog(returnData),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocListener<PurchaseReturnBloc, PurchaseReturnState>(
        listener: (context, state) {
          if (state is PurchaseReturnOperationSuccess) {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Status Update Section - Only show if not COMPLETED or CANCELLED
                      if (returnData.status != 'COMPLETED' &&
                          returnData.status != 'CANCELLED')
                        _buildStatusUpdateSection(returnData),
