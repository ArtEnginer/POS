...
class PurchaseReturnBloc
  ...
  final UpdatePurchaseReturnStatus updatePurchaseReturnStatus;
  ...
  PurchaseReturnBloc({
    ...
    required this.updatePurchaseReturnStatus,
    ...
    on<event.UpdatePurchaseReturnStatusEvent>(_onUpdatePurchaseReturnStatus);
  ...
  Future<void> _onUpdatePurchaseReturnStatus(
    event.UpdatePurchaseReturnStatusEvent ev,
    Emitter<PurchaseReturnState> emit,
  ) async {
    emit(const PurchaseReturnLoading());

    final result = await updatePurchaseReturnStatus(ev.id, ev.status);

    result.fold(
      (failure) => emit(PurchaseReturnError(failure.message)),
      (purchaseReturn) => emit(
        PurchaseReturnOperationSuccess(
          'Status return berhasil diubah menjadi ${ev.status}',
          purchaseReturn,
        ),
      ),
    );
